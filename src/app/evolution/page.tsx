...
import { History, Milestone, Briefcase, GraduationCap } from "lucide-react";
    ...
    <PageLayout>
      <section className="py-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-lg mb-24">
          {[
            { label: "YEARS_OF_DEV", value: "12+", icon: History },
            { label: "TITLES_SHIPPED", value: "30+", icon: Milestone },
            { label: "CURRENT_FOCUS", value: "XR_ECS", icon: Briefcase },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
                ...
                >
                  ...
                  <div className={`w-full md:w-[45%] ${i % 2 === 0 ? "md:pl-16" : "md:pr-16"}`}>
                    <div className={`glass-panel p-xl rounded-2xl relative group hover:tech-glow transition-all duration-500 ${
                      ...
                      {/* Optional: Add a brief placeholder for key achievements if we decide to expand data later */}
                      <div className="mt-lg pt-lg border-t border-outline-variant/20 hidden group-hover:block transition-all">
                        <p className="text-body-sm text-outline leading-relaxed italic">
                          Deployment of high-performance interactive systems and architectural scaling for global audiences.
                        </p>
                      </div>
                    </div>
                  </div>
